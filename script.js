...
// ── TRACKER: GET DOT ABSOLUTE POSITION ────────
function getDotY(nodeEl) {
  const dot     = nodeEl.querySelector('.tk-dot');
  const wrapRect = tracker.getBoundingClientRect();
...
function updateTracker(idx) {
  ...
  // Progress line: from first dot to current dot
  const firstY = getDotY(tkNodes[0]);
  const height = Math.max(0, targetY - firstY);
  tkProg.style.top = firstY + 'px';
  tkProg.style.height = height + 'px';
...
// Init tracker
setTimeout(() => updateTracker(0), 150);
...
(function spawnHomeParticles() {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const left = Math.random() * 55; // left half
    const colors = ['rgba(212,160,23,.3)','rgba(232,150,12,.2)','rgba(255,210,51,.2)','rgba(224,124,90,.15)'];
    Object.assign(p.style, {
      width: sz + 'px', height: sz + 'px',
      left: left + '%',
      background: colors[Math.floor(Math.random() * colors.length)],
      animation: `hFloat ${dur}s ${delay}s linear infinite`,
      opacity: '0',
    });
...
slides.forEach(s => {
  cHTML += `<div class="carousel-slide"><img src="${s.img}" alt="${s.title}" onclick="lbox(this.src)"><div class="carousel-caption"><h4>${s.title}</h4><p>${s.desc}</p></div></div>`;
});
...
carouselEl.addEventListener('mouseenter', () => clearInterval(cAuto));
carouselEl.addEventListener('mouseleave', () => { cAuto = setInterval(() => goSlide(cIdx + 1), 4200); });

let touchX = 0;
carouselEl.addEventListener('touchstart', e => { touchX = e.touches[0].clientX; });
...
<div class="card" onclick="openM('piracy')">
  <img class="card-img" src="${I.thumb_piracy}" alt="Piracy">
  <div class="card-body">
...
<div class="card" onclick="openM('behavior')">
  <img class="card-img" src="${I.thumb_behavior}" alt="Behaviour">
  <div class="card-body">
...
<div class="card" onclick="openM('service')">
  <img class="card-img" src="${I.thumb_service}" alt="Service">
  <div class="card-body">
...
<div class="card" onclick="openM('shell')">
  <img class="card-img" src="${I.thumb_shell}" alt="Shell">
  <div class="card-body">
...
<div class="card" onclick="openM('wedding')">
  <img class="card-img" src="${I.thumb_wedding}" alt="Wedding">
  <div class="card-body">
...
const P = {
  piracy: {
    cat:'Virtual Social Behaviour', t:'Piracy in Telegram',
...
<img src="${I.p1}"><h3>What Rules the World Today?</h3>
<p>Information/content. This information is often gatekept and considered Intellectual Property. Gatekeeping information and monetizing it makes it inaccessible to all classes of people.</p>
<img src="${I.p2}"><h3>Triangle of Digital Responsibility</h3>
<p>Piracy as acceptable and normative behavior. Blurred line between ethical and unethical download.</p>
<img src="${I.p3}"><h3>Two Schools of Thought</h3>
<p><strong>Known/accepted deviancy:</strong> It isn't theft, it's duplication. Gateway for less fortunate people to access high end.</p>
<p><strong>Pain to creators:</strong> Eats away profits. "Get everything for free" mindset.</p>
<img src="${I.p4}"><h3>Why Does One Indulge in Piracy?</h3>
...
<img src="${I.p9}"><img src="${I.p10}"><img src="${I.p11}"><img src="${I.p12}"><img src="${I.p13}">`
  ...
  behavior: {
    cat:'Behavioural Design', t:'Clean Hand Practice Among Street Food Vendors',
...
<img src="${I.s1}"><h3>Overview</h3>
<p>People don't lack options. They lack structure, reflection, and emotional grounding.</p>
<h3>5W Method</h3><p>Final chosen need: "The need to become decisive."</p>
<img src="${I.s3}"><h3>Understanding Decision Making</h3>
...
<img src="${I.wl1}"><h3>The Problem</h3>
...
<img src="${I.wl3}"><h3>Stakeholder Ecosystem</h3>
...
<img src="${I.wl4}"><h3>Key Insights</h3>
...
<img src="${I.wl5}"><h3>Wedlink Platform</h3>
...
<img src="${I.wl6}"><h3>User Flows</h3>
...
<img src="${I.wl8}"><img src="${I.wl9}"><h3>Solution Framework</h3>
...
function openM(key) {
  ...
  mInner.innerHTML = `
    ...
    ${p.hero ? `<img class="m-hero-img" src="${p.hero}">` : ''}
...
if (document.getElementById('mobAbBtn')) {
  document.getElementById('mobAbBtn').onclick = openAbout;
}
...
document.querySelectorAll('.sam-float-wrap').forEach(wrap => {
...
(function homeParallax() {
  ...
  sec.addEventListener('mousemove', e => {
    ...
    sam.style.transform = `translateY(var(--float-y, 0px)) rotate(-1deg) translate(${x}px,${y}px)`;
  });
...
// ── INJECT floatY CSS VARIABLE TRICK ──────────
// The home-samosa animation uses CSS floatY; mouse override works by removing anim temporarily
const extraStyle = document.createElement('style');
extraStyle.textContent = `
  .home-samosa:hover { animation-play-state: paused }
`;
