...
for(let i=0;i<25;i++){
  ...
  p.style.background=['rgba(212,160,23,.3)','rgba(232,150,12,.25)','rgba(255,210,51,.2)','rgba(224,124,90,.2)'][Math.floor(Math.random()*4)];
  hpEl.appendChild(p);
}

// ═══ BG SHAPES for all sections ═══
document.querySelectorAll('.bg-shapes').forEach(c=>{
  for(let i=0;i<6;i++){
    const s=document.createElement('div');s.classList.add('bg-shape');
    const sz=40+Math.random()*120;s.style.width=sz+'px';s.style.height=sz+'px';
    s.style.left=Math.random()*100+'%';s.style.top=Math.random()*100+'%';
    s.style.animationDelay=Math.random()*5+'s';s.style.animationDuration=(5+Math.random()*6)+'s';
    s.style.background=`radial-gradient(circle,rgba(212,160,23,.08),transparent)`;
    c.appendChild(s);
  }
});

// ═══ CAROUSEL — personal photo added as slide ═══
const slides=[
...
// ═══ CARDS — no images, text only ═══
document.getElementById('flCards').innerHTML=`
<div class="card" onclick="openM('piracy')"><span class="card-cat">Virtual Social Behaviour</span><h3 class="card-t">Piracy in Telegram</h3><p class="card-d">How social behaviours exist in virtual platforms and how the interface facilitates such behaviour</p><div class="card-arr">Read more &rarr;</div></div>
<div class="card" onclick="openM('behavior')"><span class="card-cat">Behavioural Design</span><h3 class="card-t">Clean Hands Among Food Vendors</h3><p class="card-d">Understanding and influencing hygiene behaviour through design interventions</p><div class="card-arr">Read more &rarr;</div></div>`;

document.getElementById('dfCards').innerHTML=`
<div class="card" onclick="openM('service')"><span class="card-cat">Service Design</span><h3 class="card-t">Empowering Student Decisions</h3><p class="card-d">Designing a service ecosystem for clarity in decision-making</p><div class="card-arr">Read more &rarr;</div></div>
<div class="card" onclick="openM('shell')"><span class="card-cat">User Research</span><h3 class="card-t">UX Research at Shell</h3><p class="card-d">Strengthening Service Design and UXR practices at enterprise scale</p><div class="card-arr">Read more &rarr;</div></div>
<div class="card" onclick="openM('wedding')"><span class="card-cat">UXR & Service Design</span><h3 class="card-t">Wedding Workforce</h3><p class="card-d">Technology as a bridge for fair pay and dignity in Indian weddings</p><div class="card-arr">Read more &rarr;</div></div>`;

// ═══ PROJECTS — hero image in summary, full images only on expand ═══
const P={
...
service:{cat:'Service Design',t:'Empowering Students to Attain Clarity',
  ...
  hero:I.s4,
  summary:'How service design can reduce ambiguity, mental overload, and emotional conflict in decision-making. People don\'t lack options; they lack structure, reflection, and emotional grounding. We designed a service framework with guided sessions, cue cards, and reflective tools.',
...
shell:{cat:'User Research',t:'UX Research at Shell',
  ...
  hero:I.sh2,
  summary:'Strengthening the role of Service Design and User Research within Shell\'s Experience Design team by addressing gaps in design maturity. Through interviews with 7 seasoned practitioners, we identified key barriers and co-created solutions benchmarked against IBM, Microsoft, and Atlassian.',
...
wedding:{cat:'UXR & Service Design',t:'Reimagining the Wedding Workforce',
  ...
  hero:I.e3,
  summary:'Designing a digital platform for fair work and payment transparency in the Indian wedding industry. With 8 to 10 million weddings annually, we mapped stakeholders, identified informal worker challenges, and proposed policy, technology, and industry solutions.',
...
// ═══ ABOUT — corrected experiences and image assignments ═══
...
<div class="ab-section">
  ...
  <div class="tl"><span class="dt">Mar 2024 to Mar 2025</span><div class="rl">Service Designer & Coordinator</div><div class="og">Luxury Event Decor Company, Hyderabad</div><div class="ds">Gained hands-on experience in high-end event decor operations. Created and defined internal work libraries and systems to formalise and simplify work.</div></div>
...
<div class="ab-section">
  <div class="ab-2col"><div>
    ...
    <h3 class="ab-st" style="margin-top:1.2rem">Skills</h3>
...
function updateTracker(a){
  ...
  // Progress bar: match to item positions
  if(jProg&&items.length>1){
    const track=document.querySelector('.sb-track');
    const first=items[0].offsetTop;
    const last=items[items.length-1].offsetTop+items[items.length-1].offsetHeight/2;
    const current=items[a].offsetTop+items[a].offsetHeight/2;
    const pct=(current-first)/(last-first);
    jProg.style.height=(pct*(last-first))+'px';
  }
}
...
// ═══ MODAL — summary first, expand hides summary and shows full ═══
...
// Magnetic snap to clickable elements
const clickables='a,button,.card,.pill,.t-item,.sb-btn,.social-link,.dl-btn,.c-link,.m-btn,.carousel-dot,.carousel-slide img';
const labels={'.card':'View project','.t-item':'Navigate','.sb-btn':'Click','.social-link':'Connect','.dl-btn':'Download','.c-link':'Open','.pill':'Explore','.carousel-slide img':'Zoom','a':'Open','button':'Click'};

document.addEventListener('mouseover',e=>{
  const el=e.target.closest(clickables);
  if(el){
    cursor.classList.add('hovering');
    // Find label
    let lbl='Click';
    for(const[sel,txt] of Object.entries(labels)){if(el.matches(sel)){lbl=txt;break}}
    cLabel.textContent=lbl;
    // Magnetic pull
    const r=el.getBoundingClientRect();
    const ecx=r.left+r.width/2,ecy=r.top+r.height/2;
    const dist=Math.hypot(mx-ecx,my-ecy);
    if(dist<120){mx=ecx;my=ecy}
  }
});
document.addEventListener('mouseout',e=>{
  const el=e.target.closest(clickables);
  if(el){cursor.classList.remove('hovering');cLabel.textContent=''}
});
